...
class HuggingFaceProvider(AIProvider):
    """Vision provider using HuggingFace Inference API (free, OpenAI-compatible)."""
    ...
    async def extract_structured(
        ...
        url = f"https://router.huggingface.co/hf-inference/models/{model_name}/v1/chat/completions"
        headers = {
        ...
        logger.info(f"Calling HuggingFace model '{model_name}' with {len(image_bytes) if image_bytes else 0} image bytes")
        ...
        if resp.status_code != 200:
            ...
            logger.error(f"HuggingFace API error {resp.status_code}: {error_text}")
            raise Exception(f"HuggingFace API error {resp.status_code}: {error_text}")

        resp_json = resp.json()
        raw_text = resp_json["choices"][0]["message"]["content"]
        logger.info(f"HuggingFace raw response: {raw_text[:500]}")
        ...
        try:
            ...
            logger.error(f"Failed to parse HuggingFace response: {e}. Raw: {raw_text[:500]}")
            raise Exception(f"Failed to parse HuggingFace response: {e}")
